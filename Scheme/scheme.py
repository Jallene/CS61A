...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda operator: scheme_eval(operator, env))
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            #Call self.fn on all of those arguments using *args notation.
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    newLambda = LambdaProcedure(formals, expressions.second, env)
    return newLambda
    # END PROBLEM 9
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** YOUR CODE HERE ***"
            if expressions.first.second is nil:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if binding.first not in env.bindings:
            symbol_list.append(binding.first)
        if binding.second.first in symbol_list:
            raise SchemeError
        else:
            vars = Pair(binding.first, vars)
            args = Pair(scheme_eval(binding.second.first, env), args)
            bindings = bindings.second
    env = env.make_child_frame(vars,args)
    return env
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    newMu = MuProcedure(formals, expressions.second)
    return newMu
    # END PROBLEM 16
