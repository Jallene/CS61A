    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.queen:
                return None    #nothing happens if we try to remove QueenAnt
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while i < self.min_range:
            ...
            s = area.bees
            i += 1
        while not s and i < self.max_range:
            area = area.entrance
            s = area.bees
            i += 1
            if area == hive:
                return None         
        return random_or_none(s)
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        #reduce a random bee's armor to 0 (the bee is in self.place)
    ...
    def action(self, colony):
        ...
        #Otherwise, eat a random Bee in its place by 
...
# END Problem 13
    ...
    implemented = True   # Change to True to view in the GUI
    food_cost = 7
    one_Queen = 0 #zero initially, then changes with more calls to QueenAnt
    doubled_ants = []
    imposter = False
    # END Problem 13
    ...
    def action(self, colony):
        ...
        in her tunnel.  Attacks nearest bee and doubles the damage of all the ants behind her (that haven't been already doubled)
        ...
        if self.imposter:
            ...
            ScubaThrower.action(self, colony)
              #include stuff about doubling damage of ants behind, 
              #if they haven't already been doubled... inc ants inside their containers
            area = self.place.exit 
            while area is not None:
                if area.ant and area.ant not in self.doubled_ants:
                    area.ant.damage = area.ant.damage*2
                    self.doubled_ants.extend([area.ant])
                if area.ant and area.ant.container and area.ant.ant and area.ant.ant not in self.doubled_ants: 
                    area.ant.ant.damage = area.ant.ant.damage*2
                    self.doubled_ants.extend([area.ant.ant])
                area = area.exit
